...
//	private int qtdeAcompanhante;
	...
	public void Conectar(String mensagem) {
		...
		// fazendo a conexão
		con1 = DriverManager.getConnection( "jdbc:odbc:california","root","" );
		}
	...
	{	
		...
		try{
		 ...
		 ps1=con1.prepareStatement("insert into reserva(cpfHosp, dataChegadaPrevista, horaChegadaPrevista, dataSaidaPrevista, horaSaidaPrevista, multa, desconto, codigoAcomodacao, precoDaDiaria, qdteAcomanhante) values( ?,?,?,?,?,?,?,?,?,?)");
	...
	{	
		...
		try{
			 ...
			 ps1=con1.prepareStatement("update reserva set dataChegadaPrevista=?, horaChegadaPrevista=? , dataSaidaPrevista= ?,	horaSaidaPrevista= ?, precoDaDiaria= ?, desconto= ?, where cpfHosp= ? and codigoAcomodacao= ?");
